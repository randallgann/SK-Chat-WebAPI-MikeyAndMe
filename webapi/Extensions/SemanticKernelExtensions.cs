...
// Re-locate this
public class GCPSecretsOptions
{
...
internal static class SemanticKernelExtensions
{
    ...
    public static Kernel RegisterChatPlugin(this Kernel kernel, IServiceProvider sp)
    {
        ...
        kernel.ImportPluginFromObject(
            new ChatPlugin(
                ...
                transcriptSearchService: sp.GetRequiredService<ITranscriptSearchService>(),
    ...
    /// <summary>
    /// Register the question generation plugin with the kernel.
    /// </summary>
    public static Kernel RegisterQuestionGenerationPlugin(this Kernel kernel, IServiceProvider sp)
    {
        var qrepo = sp.GetRequiredService<IQuestionRepository>();
        var poptions = sp.GetRequiredService<IOptions<PromptsOptions>>();
        var logger = sp.GetRequiredService<ILogger<QuestionGenerationPlugin>>();
        var sservice = sp.GetRequiredService<ITranscriptSearchService>();
        kernel.ImportPluginFromObject(
            new QuestionGenerationPlugin(
                kernel,
                sservice,
                qrepo,
                poptions,
                logger),
            nameof(QuestionGenerationPlugin));
    ...
    private static Task RegisterChatCopilotFunctionsAsync(IServiceProvider sp, Kernel kernel)
    {
        ...
        //kernel.RegisterQuestionGenerationPlugin(sp);
